<template>
    <div class="drawer_container">
        <div >
            <div class="drawer_title">表设置</div>
            <div class="drawer_wrap">
                <Form label-position="left" :label-width="80">
                    <FormItem label="表名">
                        <Input v-model="drawerNode.tableName" ></Input>
                    </FormItem>
                    <FormItem label="字段名">
                        <Input v-model="drawerNode.portName" ></Input>
                    </FormItem>
                    <FormItem label="字体类型">
                        <Input v-model="drawerNode.portType" ></Input>
                    </FormItem>
                    <FormItem label="功能">
                        <Button type="primary" icon="md-trending-up" @click="addEr">新建</Button>
    ...
    watch:{
        selectCell:{
            handler(val) {
                if(val){
                    if(val.isNode()){ //节点
                        this.drawerNode.tableName = val.label
                        this.drawerNode.portName = val.ports.items[0].attrs.portNameLabel.text
                        this.drawerNode.portType = val.ports.items[0].attrs.portTypeLabel.text
    ...
    methods: {
        addEr(){
            const node = {
                "id": "11",
                "shape": "er-rect",
                "label": this.drawerNode.tableName,
                "width": 150,
                "height": 24,
                "position": {
                    "x": 10,
                ...
                "ports": [{
                    ...
                    "attrs": {
                        "portNameLabel": {
                        "text": this.drawerNode.portName
                        },
                        "portTypeLabel": {
                        "text": this.drawerNode.portType
                        }
                    }
                }]
            }
            this.$emit('add-er',node)
        },
        // 网格设置
        changeGrid(){
